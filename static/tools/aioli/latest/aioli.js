...
class Aioli
{
    ...
    downloadText(path)
    {
        return this.send("downloadText", path);
    }
    downloadBinary(path)
    {
        return this.send("downloadBinary", path);
    }
    symlink(newpath)
    {
        return this.send("symlink", newpath);
    }
    write(file){
        return this.send("write", file);
    }
    rm(file){
    ...
    static async transfer(oldpath, newpath, workerFrom, workerTo)
    {
        ...
        return Promise.all([
            workerFrom.send("transfer", { role: "sender", path: oldpath, port: channel.port1 }, null, [channel.port1]),
            workerTo.send("transfer", { role: "receiver", path: newpath, port: channel.port2 }, null, [channel.port2])
        ]);
    }
