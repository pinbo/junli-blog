...
call_indels(
  fastq_dir = "../test/",  # the folder with all your demultiplexed fastq files
...
call_indels = function(fastq_dir, ref_fa, read1_suffix="_R1_001.fastq.gz", read2_suffix="_R2_001.fastq.gz", outdir="results", detectSV = TRUE){
  ...
  out_indel = paste0("Summary_indels_less_than_16bp_", Sys.Date(), ".csv")
  out_sv = paste0("Summary_indels_more_than_16bp_", Sys.Date(), ".csv")
  write.csv(indels,  out_indel, row.names = F)
  write.csv(newdata, out_sv, row.names = F)
  cat("Summary_indels_less_than_16bp.csv and Summary_indels_more_than_16bp.csv are created.\n\n")
