...
// v7: based on v6; added barley hits.
...
const dataPromise1 = fetch("https://jzseqbucket.s3.us-east-2.amazonaws.com/Kronos_Homeolog_with_function7.db.gz").then(res => res.arrayBuffer()).then(raw => pako.inflate(raw));
const dataPromise2 = fetch("https://jzseqbucket.s3.us-east-2.amazonaws.com/CS_Homeolog_with_function7.db.gz").then(res => res.arrayBuffer()).then(raw => pako.inflate(raw));
...
Left Join HvID d5 On ( d5.gene = d2.gene ) \
...
ORDER BY d1.name;";
    ...
    document.getElementById("thead").innerHTML = "<th>WheatGeneID</th> \
        ...
        <th>HORVU.MOREX.r3</th> \
        <th>Hv %identity</th> \
        <th>Hv align length</th> \
        ";
